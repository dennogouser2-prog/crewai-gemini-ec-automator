...
try:
    ...
    st.error("🚨 Secretsの設定（TOML形式）を確認してください。")
    st.stop()

# 2026年3月現在の最新安定モデル（1.5-flashは廃止済みです） [1, 2]
...
if st.button("コピー生成開始", key="f_p_submit") and name_input:
    ...
    with st.spinner("Tier 1のパワーでAIエージェントが作業中..."):
        try:
            # 1. 商品特性アナリスト
            analyst = Agent(
                role='商品特性アナリスト',
            ...
            copywriter = Agent(
                ...
                backstory='SEOタイトル配置技術（前半15文字に重要キーワード）のプロ [3]。',
                llm=native_llm,
                max_iter=1,
                verbose=True
            )
