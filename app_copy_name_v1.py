    ...
    with st.spinner("AIエージェントが複数の案を考案中..."):
        try:
            ...
            task2 = Task(
                ...
                3. {l3}文字以内
                4. {l4}文字以内
                各案は比較しやすいように番号付きの箇条書きで出力してください。また、単なる要約ではなく、ターゲットの心に刺さる、思わずクリックしたくなるキャッチコピーを生成してください。""",
...
with st.sidebar:
    st.divider()
    draw_text_on_image = st.checkbox("画像に商品名を印字する", value=False, key="cfg_draw_text")
    selected_text_color = st.color_picker("文字色の選択", "#000000", key="cfg_text_color")
...
if image_input:
    ...
    # 印字するかどうかのフラグを渡す
    p_name = name_input if draw_text_on_image else ""
