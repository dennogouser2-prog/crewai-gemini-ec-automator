...
if st.button("コピー生成開始") and name_input:
    ...
    # LLMインスタンスの定義import streamlit as st
import os
from crewai import Agent, Task, Crew, Process, LLM
from tools import product_web_research, background_removal_and_resize
...
# ここで os.environ =... と書くと AttributeError になるので注意
os.environ = "sk-dummy-key-for-validation"
...
if st.button("コピー生成開始") and name_input:
    ...
    # LLMインスタンスの定義
    # api_keyを明示的に渡すことで環境変数エラーを回避します
    native_llm = LLM(
        model=MODEL_NAME,
        api_key=api_key,
        temperature=1.0
    ...
    with st.spinner("AIエージェントが連携して動作中..."):
        try:
            ...
            copywriter = Agent(
                ...
                backstory='SEOタイトル（前半15文字に重要キーワード）の配置技術を熟知したプロ [1]。',
                llm=native_llm,
                max_iter=1,
                verbose=True
            )
            ...
            task2 = Task(description="指定文字数で4つのコピー案を出せ", expected_output="4案のコピー", agent=copywriter, context=[task1])
